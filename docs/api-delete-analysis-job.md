...
## 0. 现象：前端 DELETE 返回 405

浏览器里对 `/api/analysis/jobs/{id}` 发 DELETE 得到 **405 Method Not Allowed**，说明：

- 该路径上已有路由（通常是 GET），但**没有注册 DELETE**；
- 需要后端增加 `MapDelete`（见下文），而不是改前端路径。
