  ...
  return (
    <Modal open={open} onClose={onClose} title="规则与运行配置" description="配置分析 runtime、接口参数、字段规则与深度特征。" size="xl" footer={footer}>
      <div className="mb-4 flex gap-1 rounded-full bg-muted p-1 shadow-[var(--elev)]">
        {TABS.map((t) => (
          <button key={t.key} type="button" className={cn('flex-1 rounded-full px-3 py-1.5 text-sm transition-colors', tab === t.key ? 'bg-card text-foreground shadow-[var(--elev)]' : 'text-muted-foreground hover:text-foreground')} onClick={() => setTab(t.key)}>{t.label}</button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <section className="rounded-xl border border-border bg-muted/30 p-4">
            ...
            <label className="mt-4 flex cursor-pointer items-start gap-3">
              ...
              <span><span className="text-sm">展开嵌套对象 (flatten)</span><span className="mt-0.5 block text-xs text-muted-foreground">将嵌套字段展平为点路径，便于按字段写规则。</span></span>
            </label>
          </section>
          <section className="rounded-xl border border-border bg-muted/30 p-4">
            ...
            <div className="mt-3 space-y-3">
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span><span className="text-sm">CSV 数字推断</span><span className="mt-0.5 block text-xs text-muted-foreground">将可解析为数字的 CSV 单元格推断为 Number。</span></span>
              </label>
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span><span className="text-sm">强制后台分析</span><span className="mt-0.5 block text-xs text-muted-foreground">小文件也走异步作业 + 进度轮询。</span></span>
              </label>
            </div>
          </section>
          <section className="rounded-xl border border-border bg-muted/30 p-4">
            <h3 className="text-sm font-medium">分析接口参数</h3>
            <p className="mt-1 text-xs text-muted-foreground">对应 POST /analyze 表单字段。</p>
                ...
                if (d.type === 'bool') {
                  return (
                    <label key={d.key} className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-background px-3 py-2.5 sm:col-span-2">
                      <input type="checkbox" className="mt-0.5 size-4 rounded border-border" checked={!!val} disabled={busy} onChange={(e) => setAnalyzeOptions?.((prev) => ({ ...(prev || {}), [d.key]: e.target.checked }))} />
                      <span><span className="text-sm font-medium">{d.label}</span><span className="mt-0.5 block text-[11px] text-muted-foreground">{d.hint}</span></span>
                    </label>
                  );
                }
                return (
                  <label key={d.key} className="block">
                    <span className="mb-1 flex flex-col gap-0.5"><span className="text-xs font-medium">{d.label}</span><span className="text-[10px] text-muted-foreground">{d.hint}</span></span>
                    <input type={d.type === 'number' ? 'number' : 'text'} min={d.min} className="h-9 w-full rounded-lg border border-border bg-background px-2.5 font-mono text-sm outline-none focus:border-primary/40" placeholder={d.placeholder} disabled={busy} value={val ?? ''} onChange={(e) => setAnalyzeOptions?.((prev) => ({ ...(prev || {}), [d.key]: e.target.value }))} />
                  </label>
                );
              })}
            </div>
      ...
      {tab === 'rules' ? (
        <div className="space-y-3">
          ...
          {rulesMode === 'visual' ? (
            ...
            <RulesEditor text={rulesText} setText={setRulesText} error={rulesError} check={rulesCheck} onValidate={onValidate} onApply={onApply} onClear={onClear} onFetchReference={onFetchReference} applied={applied} busy={busy} />
          )}
        </div>
      ) : null}
      ...
      {tab === 'features' ? (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">深度分析特征会随分析请求一并提交。</p>
          <div className="grid gap-2 sm:grid-cols-2">
            {(featureDefs || []).map((fd) => (
              <label key={fd.key} className={cn('flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-muted/30 p-3', fd.cliOnly && 'opacity-50')}>
                ...
                <span><span className="text-sm">{fd.label}</span>{fd.cliOnly ? <span className="mt-0.5 block text-[10px] text-muted-foreground">仅 CLI</span> : null}</span>
              </label>
            ))}
          </div>
        </div>
