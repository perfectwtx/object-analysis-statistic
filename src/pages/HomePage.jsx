...
export default function HomePage() {
  ...
  const score = jobs[0]?.score ?? 0;

  return (
    <div>
      <PageHeader
        title="数据是否可信，一眼能看出来。"
        subtitle="健康分、未关闭问题和最近分析都在这里。需要动手时，再进入工作台。"
        actions={
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="flex flex-col justify-between p-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 border-t border-border pt-5">
            ...
            <Kpi label="已完成分析" value={String(jobs.filter((j) => j.status === 'done').length)} />
          ...
          <ul className="divide-y divide-border overflow-hidden rounded-xl bg-card shadow-[var(--elev)]">
            {issues.slice(0, 5).map((iss) => (
              <li key={iss.id} className="flex items-start gap-3 px-4 py-3">
                <Badge tone={severityTone(iss.severity)}>{SEVERITY_LABEL[iss.severity]}</Badge>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm">{iss.title}</div>
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {iss.field} · {DIM_LABEL[iss.dimension]} · {iss.count} 条
                  </div>
                </div>
              </li>
            ))}
              ...
              <tbody>
                {jobs.slice(0, 5).map((j) => (
                  <tr key={j.id} className="border-b border-border/70 last:border-0">
                    ...
                    <td className="px-4 py-3 tabular-nums">{formatNumber(j.score, 1)}</td>
