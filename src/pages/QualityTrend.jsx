...
export default function QualityTrend() {
  ...
  const first = trend[0];
  const delta =
    last && first && last.score != null && first.score != null
  ...
  return (
    <div>
      <PageHeader
        title={t('trendTitle')}
        subtitle={source === 'api' ? t('trendSubtitleApi') : t('trendSubtitleDemo')}
        actions={
          <>
            <BackendStatus />
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl bg-card px-4 py-3 shadow-[var(--elev)]">
          <div className="text-xs text-muted-foreground">{t('latestScore')}</div>
          <div className="mt-1 text-lg font-medium tabular-nums">
            {last?.score != null ? formatNumber(last.score, 1) : '—'}
          </div>
        </div>
        <div className="rounded-xl bg-card px-4 py-3 shadow-[var(--elev)]">
          <div className="text-xs text-muted-foreground">{t('rangeDelta')}</div>
          <div className="mt-1 text-lg font-medium tabular-nums">
            {delta == null ? '—' : `${delta >= 0 ? '+' : ''}${formatNumber(delta, 1)}`}
          </div>
        </div>
        <div className="rounded-xl bg-card px-4 py-3 shadow-[var(--elev)]">
          <div className="text-xs text-muted-foreground">{t('samplePoints')}</div>
          <div className="mt-1 text-lg font-medium tabular-nums">{trend.length}</div>
        </div>
      </div>
      <Card className="p-4 sm:p-6">
        <CardTitle>{t('trendScore')}</CardTitle>
        <CardHint>{t('trendWindowHint')}</CardHint>
