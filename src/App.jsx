...
import { fetchRulesReference, getBaseUrl, health, preflight, setBaseUrl, validateRules, ASYNC_THRESHOLD_BYTES } from './api.js';
...
export default function App() {
  ...
  const { phase, busy, progress, result: asyncResult, error: asyncError, jobId, start: startAnalyze, cancel: cancelAnalysis } = useAsyncAnalyze();
  ...
  return (
    <div className="workbench">
      <aside className={`sidebar${busy ? ' is-analyzing' : ''}`} style={{ width: sidebarW }}>
        <div className="brand">
          <div className="logo"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{ stroke: 'var(--on-gradient)' }} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v18h18" /><path d="M7 15l4-6 4 3 5-8" /></svg></div>
          <div><div className="brand-name">对象分析统计</div><div className="brand-sub">Object Analyzer</div></div>
        </div>
        <div className="side-section">
          ...
          <div className={`api-status ${apiState.status}`}>
            ...
            {apiState.status === 'ok' && <span>后端就绪{apiState.version ? ` · v${apiState.version}` : ''}</span>}
            {apiState.status === 'error' && <span className="api-err">{apiState.error}<button type="button" className="btn-link" onClick={checkApi}>重试</button></span>}
          </div>
          <details className="api-features"><summary>深度分析（P4）</summary>
        ...
        <div className="side-section">
          ...
          <div className="source-current" title={fileName}><span className="source-dot" /><span className="mono source-name">{fileName || '未选择'}</span></div>
          {error && <div className="error side-error">{error}</div>}
          <div className="side-actions">
            <button className="btn primary block" onClick={() => fileInput.current?.click()} disabled={busy}>{busy ? '分析中…' : '上传文件'}</button>
            ...
            <button className="btn block" onClick={() => source && runAnalysis(source, rulesText)} disabled={busy || !source}>重新分析</button>
            <div className="side-section job-section" style={{ marginTop: 8, padding: 0, border: 'none' }}>
              <JobProgress phase={phase} progress={progress} jobId={jobId} error={asyncError} onCancel={cancelAnalysis}
                onRetry={() => source && runAnalysis(source, rulesText)} forceAsync={forceAsync} onForceAsyncChange={setForceAsync}
        ...
        <div className="side-section">
          <button className="btn block" onClick={() => setExportOpen(!exportOpen)} disabled={!result}>导出报告 ▾</button>
          {exportOpen && result && (
            <>
              <div className="export-mask" onClick={() => setExportOpen(false)} />
              <div className="export-menu">
                {['json','csv','md','html','schema'].map((k) => (
                  <button key={k} className="export-item" onClick={() => exportReport(k)}>{k.toUpperCase()}</button>
                ))}
              </div>
            </>
          )}
      ...
      <main className="main">
        <div className="topbar">
          <div><h1>分析概览</h1><p className="subtitle">由 ObjectAnalyzer.Api（.NET）执行分析</p></div>
            ...
            <div className="panel tabs-panel">
              ...
              <div className="tab-content">
                <ErrorBoundary key={tab}>
                  ...
                  {tab === 'quality' && (rules ? <QualityPanel violations={result.qualityViolations || []} bare /> : <div className="empty-tab">尚未应用规则。</div>)}
                </ErrorBoundary>
              </div>
            </div>
          </>
